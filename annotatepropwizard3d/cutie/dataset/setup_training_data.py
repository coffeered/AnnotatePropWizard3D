...
from annotatepropwizard3d.cutie.dataset.static_dataset import SyntheticVideoDataset
from annotatepropwizard3d.cutie.dataset.vos_dataset import VOSMergeTrainDataset
from annotatepropwizard3d.cutie.utils.load_subset import load_subset, load_empty_masks
...
def worker_init_fn(worker_id):
    ...
    log.debug(
        f"Worker {worker_id} re-seeded with seed {worker_seed} in rank {_local_rank}"
    )
...
def setup_main_training_datasets(cfg, max_skip):
    ...
    dataset_configs = {
        name: {
            ...
            "empty_masks": load_empty_masks(d_cfg.empty_masks)
            if d_cfg.empty_masks
            else None,
