...
from annotatepropwizard3d.cutie.model.train_wrapper import CutieTrainWrapper
from annotatepropwizard3d.cutie.model.utils.parameter_groups import get_parameter_groups
from annotatepropwizard3d.cutie.model.losses import LossComputer
from annotatepropwizard3d.cutie.utils.log_integrator import Integrator
from annotatepropwizard3d.cutie.utils.image_saver import vis
from annotatepropwizard3d.cutie.utils.logger import TensorboardLogger
from annotatepropwizard3d.cutie.utils.time_estimator import TimeEstimator
...
class Trainer:
    def __init__(
            ...
            str(
                sum(
                    [
                        param.nelement()
                        for param in filter(
                            lambda p: p.requires_grad, self.cutie.parameters()
                        )
                    ]
                )
            ),
        ...
        parameter_groups = get_parameter_groups(
            self.cutie, stage_cfg, print_log=(local_rank == 0)
        )
    ...
    def do_pass(self, data, it=0):
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.cutie.parameters(), self.clip_grad_norm
            )
