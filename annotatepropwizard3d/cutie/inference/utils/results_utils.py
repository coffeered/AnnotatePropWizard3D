...
from annotatepropwizard3d.cutie.utils.pano_utils import ID2RGBConverter
...
from annotatepropwizard3d.cutie.inference.object_info import ObjectInfo
    ...
    def __init__(
        ...
        *,
        ...
        if "burst" in self.dataset:
            ...
            self.video_json = {
                k: v for k, v in init_json.items() if k != "segmentations"
            }
...
def save_result(queue: Queue):
    while True:
                ...
                for id in all_obj_ids:
                    ...
                    if segment_mask.sum() > 0:
                        coco_mask = mask_util.encode(
                            np.asfortranarray(segment_mask.numpy())
                        )
        ...
        if saver.save_scores:
            ...
            if last_frame:
                tmp_to_obj_mapping = {
                    obj.id: tmp_id for obj, tmp_id in tmp_id_to_obj.items()
                }
                hkl.dump(
                    tmp_to_obj_mapping,
                    path.join(this_out_path, f"backward.hkl"),
                    mode="w",
                )
        ...
        if saver.visualize:
            ...
            Image.fromarray(blend).save(
                path.join(this_vis_path, frame_name[:-4] + ".jpg")
            )
...
def make_zip(dataset, run_dir, exp_id, mask_output_root):
    if dataset.startswith("y"):
        ...
        shutil.make_archive(
            path.join(run_dir, f"{exp_id}_{dataset}"), "zip", mask_output_root
        )
