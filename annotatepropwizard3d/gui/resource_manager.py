...
class ResourceManager:
        ...
        while True:
            ...
            if args.type == "mask":
                ...
                if vis_mode == "rgba":
                    ...
                    cv2.imwrite(
                        path.join(self.visualization_dir, vis_mode, args.name + ".png"),
                        data,
                    )
                else:
                    data = cv2.cvtColor(args.data, cv2.COLOR_RGB2BGR)
                    cv2.imwrite(
                        path.join(self.visualization_dir, vis_mode, args.name + ".jpg"),
                        data,
                    )
                ...
                for i in range(1, num_channels):
                    ...
                    cv2.imwrite(
                        path.join(self.soft_mask_dir, f"{i}", args.name + ".png"), data
                    )
            else:
                raise NotImplementedError
            queue.task_done()
    ...
    def _get_image_unbuffered(self, ti: int):
        ...
        image = Image.open(path.join(self.image_dir, self.names[ti] + ".jpg")).convert(
            "RGB"
        )
    ...
    def import_layer(self, file_name: str, size: Tuple[int, int]):
        ...
        image = np.pad(
            image,
            (
                (pad_h, size[0] - new_h - pad_h),
                (pad_w, size[1] - new_w - pad_w),
                (0, 0),
            ),
            mode="constant",
            constant_values=0,
